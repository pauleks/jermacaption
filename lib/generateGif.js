...
const getPage = async () => {
    ...
    const browser = await puppeteer.launch({ product: 'chrome', args: ['--no-sandbox'] });
...
const caption = (text, identifier, gif) => new Promise(async (resolve, reject) => {
    try {
        const width = await probe(require('fs').createReadStream(gif)),
            html = getHTML(text, width);

        const page = await getPage();
        await page.setContent(html, { waitUntil: 'networkidle0' });
        await page.setViewport({ width: width.width, height: 10 });
        const elem = await page.$('div'),
            boundingBox = await elem.boxModel();
            await page.setViewport({ width: width.width, height: Math.round(boundingBox.height) + 16 });
